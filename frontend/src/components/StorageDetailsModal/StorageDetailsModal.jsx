...
import StorageDevicePreview from '../StorageDevicePreview/StorageDevicePreview.jsx'
import './StorageDetailsModal.css'

export default function StorageDetailsModal({ show, onHide, device, selectedItems, locale, onRemoveItem, onCheckout }) {
  ...
  return (
    <Modal
      ...
      size="lg"
      centered
      className="sd-details-modal"
      contentClassName="sd-details-modal__content"
      backdropClassName="sd-details-modal__backdrop"
      style={{ zIndex: 1200 }}
    >
      <Modal.Header className="border-0 pb-0 justify-content-between align-items-center">
        <Modal.Title className="fw-bold fs-5">
          {locale === 'ar' ? 'تفاصيل وحالة جهاز التخزين' : 'Storage Device Status & Details'}
        </Modal.Title>
        <button type="button" className="sd-details-modal__close-btn" onClick={onHide} aria-label="Close">
      ...
      <Modal.Body className="pt-2">
        <div className="row g-4">
          ...
          <div className="col-12 col-md-6 d-flex flex-column">
            <div className="sd-details-modal__items-card flex-grow-1">
              ...
              {selectedItems.length === 0 ? (
                ...
                <div className="sd-details-modal__sidebar-list mb-3">
                  {selectedItems.map((item) => {
                    const isGame = item.type === 'game'
                    return (
                      <div key={`${item.type}-${item.id}`} className="sd-details-modal__sidebar-item">
                        <div className="sd-details-modal__sidebar-item-name d-flex align-items-center gap-2">
                        ...
                        <div className="d-flex align-items-center gap-2">
                          <span className="sd-details-modal__sidebar-item-size text-muted small">
                            {item.size_gb ? `${Number(item.size_gb).toFixed(1)} GB` : '0 GB'}
                          </span>
                          <button
                            type="button"
                            className="sd-details-modal__sidebar-item-remove"
              ...
              {isOverCapacity && (
                <Alert variant="danger" className="py-2 px-3 small mb-3">
                  ...
                  {locale === 'ar'
                    ? 'لقد تجاوزت المساحة المتاحة! يرجى إزالة بعض العناصر.'
