  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Package Categories</h4>
      ...
      <div className="table-responsive">
        <Table striped hover className="bg-white rounded shadow-sm">
          ...
          <tbody>
            {items.length === 0 && (
              <tr><td colSpan={7} className="text-center text-muted py-3">No package categories found.</td></tr>
            )}
            {items.map(item => (
              <tr key={item.id}>
                <td>{item.id}</td>
                ...
                <td><Badge bg="primary">{item.packages_count ?? 0}</Badge></td>
                <td>
