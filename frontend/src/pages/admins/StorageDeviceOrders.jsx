...
import StorageDeviceOrderDetailsModal from '../../components/admins/StorageDeviceOrderDetailsModal.jsx'

const API_ORDERS = '/api/admin/storage-device-orders'
const TOKEN_KEY = 'authToken'
const CURRENCY = 'IQD'

const ALL_STATUSES = ['all', 'pending', 'processing', 'shipped', 'delivered', 'cancelled']

function AdminStorageDeviceOrders() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error('Failed to load storage device orders')
  ...
  const handleStatusChange = async (id, status) => {
    setUpdatingId(id)
    try {
      const res = await fetch(`${API_ORDERS}/${id}/status`, { method: 'PATCH', headers, body: JSON.stringify({ status }) })
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      const deviceName = String(item.storage_device?.name_en || item.storage_device?.name_ar || '').toLowerCase()

      return customerName.includes(q) || phone.includes(q) || id.includes(q) || deviceName.includes(q)
    })
  ...
  return (
    <>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <h4 className="fw-bold mb-0">Storage Device Orders</h4>
      ...
      <div className="row g-2 mb-3">
        <div className="col-md-8">
          <InputGroup>
            ...
            <Form.Control
              placeholder="Search by order id, customer, phone, or device"
              value={search}
        ...
        <Table striped hover className="bg-white rounded shadow-sm">
          ...
          <tbody>
            {filteredItems.length === 0 && (
              <tr><td colSpan={10} className="text-center text-muted py-3">No storage device orders found.</td></tr>
            )}
            {filteredItems.map(item => (
              <tr key={item.id}>
                ...
                <td>{item.phone || '-'}</td>
                <td>{item.storage_device?.name_en || item.storage_device?.name_ar || '-'}</td>
                <td>{item.quantity}</td>
                ...
                <td>
                  <div className="d-flex flex-column gap-2">
                    <div className="small text-muted">
                      <strong>Device:</strong> {item.storage_device?.name_en || item.storage_device?.name_ar || '-'}
                    </div>
                    {item.quantity > 0 && (
                      <div className="small text-muted">
                        <strong>Qty:</strong> {item.quantity} &times; {Number(item.unit_price ?? 0).toLocaleString()} {CURRENCY}
                      </div>
                    )}
                    ...
                    <Button size="sm" variant="outline-primary" onClick={() => handleOpenDetails(item)}>
                      View details
                    </Button>
                  </div>
                </td>
      ...
      <StorageDeviceOrderDetailsModal
        show={showDetails}
        onHide={() => {
...
export default AdminStorageDeviceOrders
